...
export const MainDashboard = () => {
    ...
    const getProductKeyValuePair = async () => {
        const response = await axios.get(
            `http://localhost:8777/hesabbook/product/key/value/get/primary/${loginData.primary_user_id}`
        );
    ...
    const getBusinessDetails = async () => {
        try {
            const response = await axios.get(`http://localhost:8777/hesabbook/business/account/all/${loginData.primary_user_id}`);
    ...
    const getPartyDetails = async () => {
        try {
            const response = await axios.get(
                `http://localhost:8777/hesabbook/partner/all/${loginData.primary_user_id}`
            );
            console.log("Party Response ", response.data.response);
            if (response.data.code === 200) {
    ...
    const getInventoryDetails = async () => {
        try {
            const response = await axios.get(
                `http://localhost:8777/hesabbook/inventory/all/${loginData.primary_user_id}`
            );
            if (response.data.code === 200) {
    ...
    const fetchSalesPurchase = async () => {
        try {
            const response = await axios.get(
                `http://localhost:8777/hesabbook/sale/purchase/all/${loginData.primary_user_id}`
            );
            console.log("Party Response ", response.data.response);
            if (response.data.code === 200) {
